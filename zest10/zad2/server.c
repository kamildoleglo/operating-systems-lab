...
void create_inet4_socket(uint16_t port) {
    Inet4_s = socket(AF_INET, SOCK_DGRAM, 0);
...
void create_local_socket(const char* name) {
    Local_s = socket(AF_UNIX, SOCK_DGRAM, 0);
...
void send_message(client_t* client, message_t message){
    sendto(client->file_descriptor, &message, sizeof(message), 0, &client->addr, client->addr_size);
}

void send_message_to_random_client(message_t message){
    int i = message.id % MAX_CLIENTS;
...
void receive_message(struct epoll_event event) {
    ...
    ssize_t bytes = recvfrom(event.data.fd, &message, sizeof(message), 0, addr, &addr_size);
            ...
            case LOGIN:
            {
                ...
                for (int i = 0; i < MAX_CLIENTS; i++) {
                    ...
                    if (Client[i].file_descriptor <= 0) {
                        ...
                        Client[i].addr = *addr;
                        Client[i].addr_size = addr_size;
                        Client[i].file_descriptor = event.data.fd;
                        Client[i].pings = 0;
                        message.type = OK;
                        send_message(&Client[i], message);
                        pthread_mutex_unlock(&Client_mutex[i]);
